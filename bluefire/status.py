...
if len(sys.argv) != 3:
    print("Usage: ./status <data folder> <epoch>")
    quit()

data_folder = sys.argv[1]
epoch = sys.argv[2]

decoder = np.load(f"./data/{data_folder}/npy/decoder.npy")[()]

state = torch.load(f"./data/{data_folder}/models/{epoch}_epoch_model_state.pytorch", map_location=lambda storage, loc: storage)
n_topics = 25
